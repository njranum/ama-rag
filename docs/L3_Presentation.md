...
> **Update — the height cap moved from the whole pair to the answer block (`M3.2-01` layout refinement, `M3.3-01` sources).** The "transcript-vs-height tension" above was first resolved by a single bounded, internally-scrolling container around the **whole** transcript (answer + source cards together). That over-shared the budget: a long answer and its (up to 4) source cards competed for the same capped box and the answer collapsed to a thin scroll slot. The cap now lives on the **answer text block alone** (`max-height: min(55vh, 480px)`, `overflow-y: auto`) — it grows with content and scrolls only when a single answer is genuinely long; the source cards sit **outside** that scroll (collapsed to a summary line by default — see Decision 5's update), so they never reduce the answer's height. The **bounded / never-shove-the-page intent is preserved** (each answer is still capped; auto-scroll-to-bottom follows the streamed tokens unless the user scrolls up); expanding sources or accumulating many pairs grows the page, which is the visitor's own deliberate action rather than the default state.

> **Update — the cap moved back onto the transcript as a single bounded window (2026-07-01, Bug-Fixes).** The per-answer cap above **left the transcript itself unbounded**: each *single* answer was capped, but multiple Q&A pairs (or a long answer once the per-answer scroll was in play) still grew the widget/page without limit — the reported "widget keeps growing" bug. Fix: the cap now lives on the **transcript container** (`max-height: min(60vh, 540px)`, `overflow-y: auto`) as the **single** bounded scroll window; the per-answer `max-height`/`overflow` is **removed** so answers flow at natural height *inside* it. The nested-scroll problem that motivated the previous update (answer vs. cards competing for one capped box) does **not** return, because (a) there's now only one scroll container, not a nested pair, and (b) sources still render as a collapsed summary line by default (Decision 5), so they add almost no height until deliberately expanded. Sticky-bottom auto-scroll moved from the answer block to the transcript container and now also fires when a **new pair commits** (not just on token growth), so the newest content stays pinned to the bottom chat-style — still released the instant the user scrolls up (`stickToBottomRef`). This restores the original Decision 6 "bounded internally-scrolling transcript" as the literal implementation. *(Also reconciles the stale `M3.7-01` note further down that still described a `min(60vh, 520px)` cap on the transcript — the cap is back on the transcript, now at `min(60vh, 540px)`.)*
...
- **The decline is *not* a distinct state.** Per the contract, the bare decline arrives as a normal `delta`+`done` — so at the data layer a decline is simply *an answer*, rendered through the same path as any other. We explicitly do **not** detect or style it specially: the only way to "detect" it would be string-matching Layer 2's canned wording, which Layer 2 reserved the right to soften — coupling the widget to exact text is fragile. *Note there are two decline shapes, and **as of 2026-07-01 both carry no `sources` event** (Layer 2 now suppresses sources on the prompt-side decline too — see Decision 5). Both are still "just an answer" at the state layer — the state model needs no special case, and the widget still never string-matches the wording.*
...
- **Prompt-side decline → now sourceless too (was: sources on screen).** There is a *second* decline path: the context cleared the retrieval gate but Haiku judged it insufficient and declined. **As of 2026-07-01 (Bug-Fixes) Layer 2 suppresses the `sources` event on this path too** — `pipeline.resolve_sources` peeks the stream and, on the exact canned decline, withholds `sources` — so the widget receives `delta`+`done` with no sources, identical to the gate decline. *Superseded reasoning (preserved):* "By then the `sources` event has already streamed … so the widget cannot retract it, and cannot detect the decline without string-matching wording Layer 2 reserved the right to change." That still holds *for the widget* — but Layer 2 matching its **own** `DECLINE_MESSAGE` is not fragile, so the fix moved server-side. The honest sources **label** (below) is retained as defence-in-depth and for the normal-answer case, but is no longer load-bearing for the decline path.
...
**Deferred:** click-to-expand the full chunk text — a nice refinement, premature; the preview establishes provenance. *(Update — `M3.3-01` refinement: a disclosure interaction now exists at the **block** level — the sources collapse to a summary line and expand to the preview cards (see the collapse note below) — which realises the click-to-expand **direction**. Expanding the **full untruncated chunk text** per card remains deferred; the leading+trailing-ellipsis preview still establishes provenance.)*

> **Update — sources render as a collapsed summary-on-arrival, expandable to the cards (`M3.3-01` + `M3.7-01`; supersedes "render the cards the instant it lands").** The original decision was *"Render sources on arrival — render the **cards** the instant the `sources` event lands; this doubles as the exit from the loading state and fills the latency gap before the first token ('found these → now composing')."* That render-on-arrival **intent is preserved**, but rendering the full grid of (up to 4) cards immediately caused a layout failure once the answer block and the cards shared **one** height-capped, internally-scrolling container (the M3.2 transcript): a long answer and the 4 cards competed for the same vertical budget and the answer collapsed into a thin scroll slot. The fix splits the two: on arrival the widget now renders a single compact **summary line** — the inverted provenance tag + a singular/plural source count + a chevron — which still serves as the loading-state exit / "now composing" cue (it appears before the first token). The line is a real `<button>` (`aria-expanded` / `aria-controls`, focus ring, Enter/Space), **collapsed by default**; expanding reveals the same grouped cards (group-by-title, `…preview…`, conditional `read more →`, all-null note suppression) unchanged. The height cap moved onto the **answer block alone** (see Decision 2's update) so an expanded sources panel grows the pair taller (page scroll) — a deliberate user action, never the default. *Count = distinct grouped cards (one per title), so the number on the line always matches what expands.*

> **Update — archive-restyle surface refresh (`M3.7-01`; decisions unchanged, presentation only).** The label *form* is unchanged — still the provenance-asserting **"From Nic's portfolio:"**, not a causation label — so the honest-on-every-path reasoning above still holds; it is now rendered as an **inverted mono block** (paper-on-ink rectangle), which asserts provenance no differently. Two presentation corrections/additions: (1) the compact preview now renders the **leading *and* trailing** ellipsis the "compact preview + ellipsis" decision specified — the M3.3 markup had emitted only the leading one, so this is the code catching up to the doc, not a new decision; (2) a **decorative monospace record index** (`01`, `02`, …) was added per card as an archive-aesthetic cue — `aria-hidden` so it never reaches AT, and purely ordinal over the already-grouped cards (no retrieval/contract meaning). Group-by-title, the conditional `read more →` (url non-null), and the all-null note suppression are all unchanged.
...
>
> **Update — archive / changelog aesthetic restyle (`M3.7-01`, supersedes the palette specifics above; strategy & isolation unchanged).** The original "pixels now chosen" note picked a *neutral-with-blue-accent* skin (`--rag-accent: #1a5fb4` links + primary button, `--rag-surface` chip fills, rounded `border-radius`, system-sans throughout). That choice still satisfied D9 but read as a generic chat box. The restyle re-skins to a **high-contrast ink-on-paper archive aesthetic**: *no accent colour* (ink + paper + 1.5px rules only), **square corners everywhere** (`border-radius: 0`), **monospace for all UI/metadata/labels** and **serif for the streamed answer body only** (sans kept as the inherited reset baseline). The superseded blue-accent reasoning is preserved above for lineage — it was never wrong, just visually anonymous; the change is taste + portfolio distinctiveness, not a correctness fix.
>   - **Palette vars renamed/retired** (still `--rag-*` on `.widget`, still host-overridable, still the D9 theming mechanism — only the *names* and *values* changed): `--rag-fg`→`--rag-ink`, `--rag-bg`→`--rag-paper`; `--rag-border`/`--rag-border-strong`/`--rag-surface`/`--rag-accent`/`--rag-on-accent` **retired** (a single `--rag-rule` = ink for the 1.5px structural rules, plus a decorative `--rag-rule-hair` hairline for the in-pair answer→sources divider; the inverted blocks fill with `--rag-ink` and set text to `--rag-paper`, so no separate on-accent var is needed). Kept: `--rag-muted`/`--rag-faint`/`--rag-error`/`--rag-focus` (focus now = ink, no accent). Added type vars `--rag-font-mono` / `--rag-font-serif` / `--rag-font-sans`.
>   - **AA re-verified both schemes** with the new values: light ink `#161513` on paper `#faf9f6` (~17:1), muted `#5a5752` (~7.2:1), faint `#6c6862` (~5.6:1), error `#8a1c1c` (~7.1:1); dark flips ink↔paper (`#f1efe9` on `#14130f`) with muted `#a8a39a` (~7.6:1) / faint `#928d84` (~5.7:1) / error `#ff8a8a` (~6.0:1). The **inverted label blocks** (ASKED tag, provenance label, ASK button) fill with ink and write in paper, so they stay ≥AA in *both* directions because both vars flip together.
>   - **New visual elements, all decorative / aria-hidden (no a11y refactor):** a mono panel **header tag** (the page `<h1>` remains the real heading); an **inverted "ASKED" tag** before each transcript question (the existing srOnly "You asked:" still carries it to AT); a **terminal input** — a "›" prompt marker + borderless field inside a 1.5px ink box with the inverted ASK button butted on via a 1.5px divider; **outlined mono chips** (1px ink, square) that invert on hover/focus under a "TRY ONE OF THESE" mono label. The chips' populate-and-send behaviour, the reducer, SSE parser, and fetch/abort policy are untouched.
>   - **Reduced motion:** the pending indicator is now a blinking terminal cursor (`▋`, `steps(1)` blink) instead of an opacity pulse; gated behind `prefers-reduced-motion: no-preference`, with a blanket `prefers-reduced-motion: reduce` override killing the blink *and* the chip hover transitions. **Focus:** the borderless input's ring moves to its wrapper (`.inputBox:focus-within`) so the field keeps a clear, single affordance — the reset still strips nothing from buttons/links (they keep the `:focus-visible` ink ring).
>
> **Update — dark mode re-gated on the HOST theme, not the OS (`M3.7-01`; supersedes the "support `prefers-color-scheme` as default behaviour" commitment above).** The original commitment was *"Dark mode via the same CSS variables — support `prefers-color-scheme` as default behaviour (cheap, since theming is already custom-property-based)."* That reasoning was sound in the abstract but wrong against **this** host: the portfolio site is **light-only** (no theme system), so an OS-level `@media (prefers-color-scheme: dark)` painted the widget dark-on-white for any visitor whose OS was in dark mode — a clash, not a feature (the widget honoured the *OS* while the page around it stayed light). The `@media (prefers-color-scheme: dark)` rule is therefore **removed**, and the dark palette is re-gated on an **ancestor host-theme scope** — `:global(.dark) .widget, :global([data-theme="dark"]) .widget` — so dark applies only when the *site* opts in. All the dark var values (and their AA checks) are **preserved verbatim**, so dark mode returns for free the moment the site grows a theme toggle. **Net:** the widget now follows the host, never the OS; dark mode is **deferred together with the site-wide theme toggle** (consistent with the existing "wiring into a *site-specific* theme toggle is deferred" half of the original commitment — that half is now the whole rule). *The underlying mechanism (one palette as overridable `--rag-*` custom properties) is unchanged — only the trigger moved from a media query to a class scope.*
>
> **Update — auto-expand transcript + start-over control (`M3.7-01` styling · `M3.2-01` RESET wiring).** **(1)** The transcript's fixed `max-height: 360px` (which scrolled almost immediately) becomes **`max-height: min(60vh, 520px)`** with `height: auto` — short/medium answers grow in place and never scroll; only very long ones become an internal scroll region, preserving Decision 2's "bounded, never shoves the page down" intent. While streaming, the component keeps the latest content in view (sets `scrollTop = scrollHeight`) **only while the user is parked at the bottom** — an `onScroll` handler flips a `stickToBottomRef` off the moment they scroll up, so streamed tokens never yank them back. **(2)** The header bar stops duplicating the page `<h1>`: its left side is now a decorative mono system tag (`Portfolio · Q&A`, aria-hidden), and its right side carries a **start-over** icon button (refresh glyph, `aria-label="Start over"`, ink/mono, square, `:focus-visible` ring) that dispatches the **existing `RESET` action** (M3.2-01 / D4) — clearing the transcript back to the empty/base state and refocusing the input. Reset reuses the existing silent abort reason to cancel any in-flight stream (no error surfaced); the button is hidden in the pristine `idle`-and-empty base state (nothing to reset yet). No change to the SSE parser, fetch, or abort logic. **(3)** The disabled ASK button was restyled from a flat filled grey block to an intentional **ghost** (muted ink text, hairline left divider, no fill) so it reads as deliberately inactive, not broken.
