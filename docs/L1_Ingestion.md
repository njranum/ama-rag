...
> **Correction at the swap (`M4.2-03`) — the "no code change" claim holds for Chroma (dev) only, NOT Pinecone (prod).** The mark-and-sweep reconcile (`ingest.sync` → `reconcile_and_sync`) is wired to the **Chroma** collection API (`get_collection`, `collection.get(include=…)`, `collection.delete(where=…)`) and does **not** branch on `config.VECTOR_STORE`, so it never touches the prod Pinecone index. The Pinecone write path (`embed_store.main` / `store_chunks_pinecone`) is an **upsert with no sweep** — running it would add the real pages but leave the synthetic vectors in place (both corpora coexisting → a launch-gate failure). So on Chroma the swap was genuinely content-only (`ingest.sync`: removed 9 synthetic, added 11 real), but the **prod Pinecone swap was done as a deliberate one-off**: `index.delete(delete_all=True)` + full re-ingest (verified: 10 synthetic → 0 → 19 real chunks, synthetic-only queries now decline below the 0.375 gate, zero synthetic remnants). Safe because the index only ever holds the Portfolio corpus. A proper **Pinecone-aware mark-and-sweep** (delete removed `page_id`s + delete-before-upsert changed) is **deferred to a follow-up ticket** so future real-content edits reconcile with `VECTOR_STORE=pinecone python -m ingest.sync` as the design intends — until then, prod re-ingests are the delete-all + reload one-off. *(Deferral chosen at `M4.2-03`; the tiny, infrequently-edited corpus makes the one-off acceptable for launch.)*
