...
> **Re-lock on casual phrasings (2026-07-01, Bug-Fixes — threshold LOWERED to 0.34).** A real-use report exposed a false refusal: *"What are nics salary expectations"* (informal `nics`, no `?`) embeds at **0.368** and the 0.375 gate hard-rejected it *before the LLM was called* — yet the salary figure is in the corpus and the **well-formed** twin ("What are Nic's salary expectations?") scores 0.459 and answers. Root cause: the M4.2-03 eval set was **all well-formed**, so the threshold was tuned to an unrealistically high should-answer floor. Fix: (1) added a **casual should-answer** block to the eval set (`_SHOULD_ANSWER_CASUAL` — how recruiters actually type; lowest is "when can nic start" **0.350**), and (2) **split the should-refuse set by owner** (`RefuseKind`): `offtopic`/injection is the **gate's** job, `about_nic`-but-unanswerable is the **decline's** job. Measuring the gap against **off-topic only** (the gate's real responsibility) makes it **clean again**: highest off-topic **0.318** < lowest should-answer **0.350** → threshold **0.340** (`lowest_answer − 0.01`), **0 off-topic leaks**. The five about-Nic-unanswerable questions (0.368–0.484) now *intentionally* clear the gate and are refused by the prompt-side decline — which is the M4.2-03 note's own stated design ("no single clean threshold … conservative gate + prompt-side backstop"), now implemented literally rather than by setting the gate at the should-answer floor. This is **not** a loosening of safety: every off-topic/injection case is still caught (they top out at 0.318, far below 0.340). Supersedes **0.375**; `config.RELEVANCE_THRESHOLD = 0.34`. Re-derive: `python -m query.calibrate`.
...
> **Update — the prompt-side decline is now *verbatim*, not "for example" (2026-07-01, Bug-Fixes).** The Stage-3 prompt originally offered the wording as an example (*"for example: …"*), which let Haiku **paraphrase** it (observed in prod: *"Sorry, I don't have that information."*). That broke `resolve_sources` (below), whose source-suppression matches the **exact** `DECLINE_MESSAGE` — a paraphrase slips through and the sources still render. The prompt now instructs the model to reply *"with exactly this sentence and nothing else"* and *"verbatim — do not paraphrase"*. This makes the two decline paths byte-identical (strengthening the consistency point above) **and** makes the sourceless-decline behaviour reliable rather than best-effort. Verified end-to-end: "star sign" / "car" now emit the exact string and drop their sources.

**Interaction with the `sources` event — both declines are now sourceless (cross-layer with Layer 3):** the **gate** decline fires *before* generation, so **no `sources` event is sent**. The **prompt-side** decline fires *during* generation, so the sources cannot simply be withheld up front — but `pipeline.resolve_sources` **peeks the token stream** and, if the answer is *exactly* the canned decline, **drops the sources** before the `sources` event is emitted. The peek costs nothing on a real answer (as soon as the text diverges from the decline prefix the sources fire and the peeked tokens replay with no added latency); only a bare, exact decline is suppressed. So **both** decline paths now reach the widget identically: `delta` + `done`, no `sources`. *(The exact-match is only reliable because the Stage-3 prompt forces the decline **verbatim** — see "Decline wording" above; with the earlier "for example" prompt the model paraphrased and sources leaked through.)*
...
**CORS:** `Access-Control-Allow-Origin` locked to the site's origin, not `*`. The allowlist must include **both** dev-loopback spellings (`http://localhost:3000` *and* `http://127.0.0.1:3000` — a browser opened on the `127.0.0.1` URL sends it as a distinct Origin, so listing only `localhost` silently blocks the other) for Phase 2 widget testing **and** the production site origin for Phase 3 — omitting the dev origin blocks local testing on a CORS wall. Not real security (the API is directly reachable — that's what rate limiting is for), but blocks casual cross-site embedding. *(Both loopback spellings added 2026-07-01, Bug-Fixes — a widget served on `127.0.0.1:3000` was hitting the CORS wall.)*
