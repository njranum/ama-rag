...
- **Real-content authoring deferred off the build's critical path** (post-design restructure): the system is built and calibrated on a **shape-matched synthetic persona**, swapped to real content as a **launch gate** (`M4.2-03`) before public exposure. The gate threshold and the suggested-question chips are therefore **provisional working choices** until re-calibrated/regenerated on real content. A clean instance of "defer, don't drop" + "working choice vs locked"; `M1.1-01` was split into `M1.1-01a` (synthetic seed, Claude) and `M1.1-01b` (real authoring + audit, Manual). *(See `Placeholder_Content_Restructure.md`.)*
